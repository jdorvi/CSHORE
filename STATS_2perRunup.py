...
#function err = STATS_2perRunup(inpth1,inpth2,outpth,tran_id,step)
#--------------------------------------------------------------------------
...
storms = [int(line.strip('\n')) for line in open(os.path.join(inpth2,'stormlist.txt'))]
Nyrs = storms[0] # number of years now inlcuded as first line in stormlist.txt - updated 3/9/15 -MFS
storms = storms[1:]

max_runup = []
outfile = os.path.join(outpth, '{}_max_runup.txt'.format(tran_id))

if step_num == 1:
    for ii in range(length(storms)):
        runup_ts = []
        swel_ts = []
        path = os.path.join(inpth1, 'TR{}'.format(tran_id), storms[ii])
        
        #open CSHORE output file containing 2 percent runup value
        try:
            with open(os.path.join(path,'ODOC'), 'r') as fid:
                for line in fid:
                    if '2 percent runup' in line:
                        runup_ts.append(line.strip().split()[-1])
                    elif 'Still water level' in line:
                        swel_ts.append(line.strip().split()[-1])
        except: # Need to make a proper exception here
            pass 

        # Write 2 percent runup to output file
...
"""
            ...
            fid == -1: #Flag if ODOC cannot open.... indicates error in running CSHORE, review input file
