"""
function err = profile_extract(inpth1,inpth2,outpth,tranid,shoreline,length)
--------------------------------------------------------------------------
...
JD 04/17/2017 translated to Python
--------------------------------------------------------------------------
INPUT
   ...
   shoreline   - shoreline elevation in feet
   length      - offshore transect length in feet
OUTPUT
...
--------------------------------------------------------------------------
 ...
 lgth='2146.826';
"""
shoreline*=0.3048   #convert to meters
length*=0.3048      #convert to meters
err=0

# Load in storm/scenario list
storms = [line.strip() for line in open(os.path.join(inpth2,'stormlist.txt'))]

# for each transect and scenario
for storm in storms:
    path=os.path.join(inpth1, 'TR{}'.format(tranid), storm)
    
    #------------------------------------------------
    # open CSHORE output profile
    try:
        with open(os.path.joint(path,'OBPROF')) as fid:
            cnt = 0
            out = []
            for line in fid:
